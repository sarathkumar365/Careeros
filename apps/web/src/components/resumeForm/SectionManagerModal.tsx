...
function SectionCard({
  ...
  return (
    <>
      <div
        className={`font-medium ${inverted ? 'text-gray-200' : 'text-black group-hover:text-gray-200'}`}
      >
        {label}
      </div>
      <div
        className={`text-sm ${inverted ? 'text-gray-200' : 'text-gray-500 group-hover:text-gray-200'}`}
      >
        ID: {id} | Style: {styleId}
      </div>
    </>
...
const DraggableSection = memo(function DraggableSection({
  ...
  return (
    <div className="relative">
      {showDropIndicator && (
        <div className="absolute -top-1 right-0 left-0 h-0.5 bg-black" />
      )}
      <div
        ref={setNodeRef}
        style={style}
        {...attributes}
        {...listeners}
        className={`group cursor-move border bg-white p-3 transition-colors hover:border-black hover:bg-black${
  ...
  return (
    <div className="space-y-2">
      ...
      <SortableContext items={itemIds} strategy={verticalListSortingStrategy}>
        ...
        >
          {isEmpty && !isOver && (
            <div className="border border-dashed p-4 text-center text-sm text-gray-500">
              {emptyMessage}
            </div>
          )}
...
export function SectionManagerModal({
  ...
  return (
    ...
    >
      <div className="mb-4">
        <h2 className="text-xl">Manage Sections</h2>
