      ...
      <div className="grid grid-cols-3 gap-2">
        {years.map((y) => (
          <button
            ...
            type="button"
            onClick={() => onSelect(y)}
            className="px-2 py-2 text-sm hover:bg-black hover:text-white"
          >
            {y}
          </button>
        ))}
      ...
      <div className="grid grid-cols-3 gap-2">
        {MONTHS.map((monthName, index) => (
          <button
            ...
            className="bg-white px-2 py-2 text-sm hover:bg-black hover:text-white"
          >
            {monthName}
          </button>
        ))}
...
export function DateInput({ id, label, value, onChange }: DateInputProps) {
  ...
  const getGranularityButtonProps = (targetGranularity: Granularity) => ({
    bgColor: granularity === targetGranularity ? 'bg-black' : 'bg-white',
    textColor: granularity === targetGranularity ? 'text-white' : 'text-black',
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        ...
        className="mb-1 w-full border-b border-gray-400 px-3 py-2 text-left text-sm focus:border-b focus:border-black focus:outline-none"
      >
        {value || NBSP}
      </button>
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute z-50 mt-1 w-64 border bg-white p-3">
          {/* Granularity Buttons */}
